...
/**
 * Created by wsj on 2018/3/28.
 */
public class OkHttpUtils {
    private static final byte[] LOCKER = new byte[0];
    private static OkHttpUtils mInstance;
    private OkHttpClient mOkHttpClient;
    private UserNetCall mUserNetCall;
    private android.os.Handler mDelivery;                           //用于在主线程执行的调度器
     ...
     * 修改自定义网络数据回调接口
     */
    public interface UserNetCall {
        void success(Call call, String json);
    ...
    public void getDataAsynToNet(String url, Map<String, String> params, final MyNetCall myNetCall) {
        String url1 = getUrl(url, params);
        //1 构造Request
        Request.Builder builder = new Request.Builder();
        Request request = builder.get().url(url1).build();
        //2 将Request封装为Call
        Call call = mOkHttpClient.newCall(request);
        //3 执行Call
        call.enqueue(new Callback() {
    ...
    public void onSuccessResponse(final Call call, final String json) {
        ...
        mDelivery.post(new Runnable() {
            @Override
            public void run() {
                if (mUserNetCall == null) return;              //请求成功回调 （UI线程）
                mUserNetCall.success(call, json);              //请求结束回调 （UI线程）
            }
        });

    }

    public void onFaildResponseData(final Call call, final IOException e) {
        if (mDelivery == null) {
            mDelivery = new Handler(Looper.getMainLooper());
        }
        mDelivery.post(new Runnable() {
            ...
            public void run() {
                if (mUserNetCall == null) return;              //请求成功回调 （UI线程）
                mUserNetCall.failed(call, e);                //请求结束回调 （UI线程）
            }
        });
    }
